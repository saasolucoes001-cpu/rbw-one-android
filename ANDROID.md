...
Os canais são `rbw_updates_bem_te_vi_v1` e `rbw_updates_silent_v1`. A preferência atual do usuário escolhe o canal. No Android 8+, a configuração do sistema para cada canal prevalece sobre o app (volume, modo Não Perturbe, som alterado/desativado pelo usuário). O áudio enviado como `RBW One Som Notificacao.mpeg` está em `app/src/main/res/raw/bem_te_vi.mp3`, sem recodificação; a origem está em `rbw_notification_source.txt`. O nome do recurso e o ID do canal são mantidos por compatibilidade com instalações existentes e preferências do sistema. É necessário compilar e instalar um novo APK para receber o áudio atualizado. Não há repetição do áudio da WebView quando a entrega nativa está ativa.
